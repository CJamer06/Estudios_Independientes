...
print(<Nombre de la lista>[<Posición>]
print(<Nombre de la lista>[- <Posición>] #En este caso se recorre de atrás hacia delante.
'''

print(f"El primer dia de la lista es {lista[0]}") #Python reconoce el 0 como primera posición
print(f"El ultimo dia de la lista es {lista[-1]}") #Al igual que el -1 como su ultima posición
...
Para ingresar datos en una lista usamos la siguiente función: 
<Nombre lista>.insert(<posición>,<Dato a agregar>)
'''
...
print(lista.index(int(input("\nDesea buscar un numero: "))),"es la posición del numero")

'''
Anexos
    <Nombre lista>.count(<Datos>): Permite saber cuantas veces hay un valor dentro de la lista
    <Nombre lista>.pop(<posición (n-1)>): Permite eliminar un dato según la posición (n-1) y si se deja vació 
